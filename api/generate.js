// POST /api/generate - 根据错题生成举一反三练习题
...
export default async function handler(req, res) {
  ...
  try {
    const { question_ids, direction, count = 3 } = req.body

    if (!question_ids || !Array.isArray(question_ids) || question_ids.length === 0) {
      return res.status(400).json({ error: '请选择至少一道题目' })
    }
    ...
    if (!questions || questions.length === 0) {
      return res.status(400).json({ error: '未找到相关题目' })
    }
...
- answer_lines: 数字，应用题答题行数，默认4`
    ...
    if (!deepseekResponse.ok) {
      ...
      return res.status(500).json({ 
        error: result.error?.message || 'AI生成失败，请重试' 
      })
    }
